...
> ### 0. Redis는 '메모리 위에 있는 고성능 창고'입니다
> 
> 보통의 데이터베이스(MySQL 등)는 하드디스크에 저장하지만, **Redis는 컴퓨터의 메모리(RAM)에 직접 데이터를 저장**합니다. 그래서 엄청나게 빠릅니다.
> 
> - **Key(키):** 창고에 있는 **상자의 이름표**입니다.
> - **Value(값):** 그 **상자 안에 든 내용물**입니다.
> - **Set(집합):** Redis의 상자 종류 중 하나로, **중복을 허용하지 않는 주머니**입니다.
> 
> ### 1. `eventId`의 정체와 역할
>
