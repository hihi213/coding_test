...
- **복잡도 (Complexity):**
    - **시간 복잡도:** 리스트를 순회하며 딕셔너리를 만드는 과정 $O(N)$, 다시 딕셔너리를 순회하며 찾는 과정 $O(N)$으로 전체적으로 **$O(N)$**입니다. $N=10^4$ 조건에서 충분히 통과합니다.
    - **공간 복잡도:** `dict1`에 모든 데이터를 저장하므로 **$O(N)$**입니다.
- **로직의 비효율성 (팩트 체크):**
...
>
> **모범 답변:** 해시 맵은 Key를 **해시 함수(Hash Function)**에 통과시켜 나온 해시값을 인덱스로 하여, 배열(Bucket)의 특정 위치에 Value를 저장합니다. 이 때문에 평균적으로 **$O(1)$**의 속도로 데이터에 접근할 수 있습니다.
...
이 로직을 실제 서비스(수백만 명의 유저가 동시에 사용하는 환경)에 배포한다고 상상해 봅시다.

서버 메모리에 모든 데이터를 올리는 게 가능할까요? 데이터가 계속 변한다면요?

**"현업/면접에선?"** 이라고 물어봐 주시면, 이 코드를 **실무 엔지니어의 아키텍처**로 발전시켜 드리겠습니다.
...
- **솔루션 제안:** "이 문제는 빠른 검색 속도와 Key-Value 구조가 핵심이므로 **`Caching Layer (Redis)`**를 사용하는 것이 정석입니다."
    ...
    - `dict.get(key)` → Redis `GET key` (복잡도 $O(1)$ 동일)
- **현업 코드 (Redis 활용):**
...
1. **동시성 이슈 (Concurrency):** `A` 서버와 `B` 서버가 동시에 같은 금액을 조회해서 처리해버리면 **이중 정산(Double Spending)**이 발생합니다.
    ...
    - _해결:_ Redis의 `SETNX` (Set if Not Exists) 명령어나 `Redisson` 같은 분산 락(Distributed Lock) 라이브러리를 사용해 한 번에 하나의 프로세스만 매칭을 처리하게 해야 합니다.
...
3. **키 충돌 (Key Collision):** 서로 다른 고객사의 입금액이 우연히 같을 수 있습니다 (`100원`).
    ...
    - _해결:_ Redis Key를 `고객사ID:입금액` 형태(Namespacing)로 설계해야 합니다. (예: `CompanyA:5000`)
...
"알고리즘 문제에선 HashMap을 직접 구현하는 게 능력이지만, 현업에선 **'이미 잘 만들어진 Redis를 가져다 쓰는 것'**이 능력입니다. 이게 바로 코더와 엔지니어의 차이입니다. 바로 Redis 설정부터 잡아볼까요?"
